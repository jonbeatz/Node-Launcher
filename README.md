...
**Packaging pipeline (v1.1.4+):** **`npm run vader:sync`** runs **`vader:dev`** with **`--success last`** so npm waits for **full** dev process teardown, then runs **`vpe:take-state-snapshot`**, **`vpe:check-readiness`**, and **`build:win`**. Full command table: **`.cursor/docs/VPE-BUILD-PROTOCOL.md`**.
