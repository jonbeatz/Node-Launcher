...
**Shipped desktop build (npm / `package.json` `version`):** **1.5.0** — preload **`vpeInfo.version`**, footer, and **`layout.tsx`** metadata must match; dev branch **`VPE-v1.5.x-Dev`** (next new line: **`VPE-v1.6.x-Dev`** — increment minor, bump **`package.json`** minor accordingly). See [.cursor/docs/VPE-BUILD-PROTOCOL.md](.cursor/docs/VPE-BUILD-PROTOCOL.md) for **`vader:*`** scripts and in-app tooling. Prior layers still apply: **`vader:clean-sync`** / **`vader:sync`**, NET dev override (**v1.2.3+**), shield + tactical filters (**v1.2.4–v1.2.5**), **`is_archived` + Ctrl+K** (**v1.2.6**).
...
**v1.3.1 (ops / branching):** Introduced **`VPE-v1.3.x-Dev`**; CI **`.github/workflows/ci.yml`** runs **`npm ci`** (requires **`package-lock.json`** synced to **`package.json`**) → lint → **`build:renderer`** → AST stub → Playwright **`test:e2e`** (lint strictness superseded by **v1.3.3** relaxed lint above). **Current (v1.5.0+):** **`VPE-v1.5.x-Dev`** — see [.cursor/docs/Checkpoint.md](.cursor/docs/Checkpoint.md) **Build v1.5.0** for branch / semver alignment.
