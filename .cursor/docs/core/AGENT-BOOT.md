...
- **New session + project setup (agents):** On **Start Project** (or equivalent), follow **[Start-Project.md](../prompts/Start-Project.md)** for the canonical paste block + numbered steps — it now states the **full stack read order** (rules → this §1/§4 → **API-SetUp-Master** → **`vpe-start-api.ps1`** + **ngrok** → log summary). After §1 here, **run start API**: from repo root in a **Cursor integrated terminal split pane**, **`.\google-api\vpe-start-api.ps1`** ([API-SetUp-Master.md](../API-SetUp-Master.md)); in **another** pane run **`ngrok http 4000`** globally (**v1.7.7** *API-setup* ritual: **ngrok** on **User PATH**, split panes only — **no** `.\ngrok.exe` / repo cwd required). **No** external windows. Do this **early**—before relying on overrides / model routing. Confirm **`[VPE STANDBY]`** then **Uvicorn** on **4000** (**API is Live**). Summarize **[VADER_STATION_LOG.md](../../VADER_STATION_LOG.md)**. If **`.\google-api\gcp_key.json`** is missing, note **blocked**; do not guess credentials.
...
| 3 | [VPE_ENGINE_CAPABILITIES.md](VPE_ENGINE_CAPABILITIES.md) | Agent behavior, hooks/prompts index, quality gate |
| 4 | [../guides/PRD.md](../guides/PRD.md) | Features and requirements |
...
| **Cursor ↔ Vertex (new session)** | **Start Project** ritual: [Start-Project.md](../prompts/Start-Project.md). **`.\google-api\vpe-start-api.ps1`** + global **`ngrok http 4000`** in **split integrated terminal panes** (**v1.7.7** = LiteLLM/ngrok runbook — [API-SetUp-Master.md](../API-SetUp-Master.md); **shipped VPE** semver = root **`package.json`**). After **`[VPE STANDBY]`**, confirm Uvicorn on **4000** → **API is Live**. Log summary: **[VADER_STATION_LOG.md](../../VADER_STATION_LOG.md)**. |
...
| Doc | Use when |
| :--- | :--- |
| [Start-Project.md](../prompts/Start-Project.md) | **Cold session ritual** — Golden Ticket (**`google-api\vpe-start-api.ps1`**), **`[VPE STANDBY]`**, **API is Live**, **VADER_STATION_LOG** summary |
| [Checkpoint.md](../guides/Checkpoint.md) | Branch status, recent milestones, risky areas, files to reopen |
| [Custom-Commands.md](../guides/Custom-Commands.md) | **`rebuild exe`**, **`Update Docs`**, **`restart app`**, **`start app`**, **`Managed project dev` (v1.2.3)**, **`hardened setup`**, Playwright MCP table, MCP sanity checklist |
...
| [Stability.md](../guides/Stability.md) | Packaging, ASAR, winCodeSign, native rebuild, telemetry—**resolved** symptoms |
...
- [ ] **Start API** — In a **Cursor integrated terminal pane** at repo root: **`.\google-api\vpe-start-api.ps1`**; in **another** pane: **`ngrok http 4000`** (global PATH). Unless LiteLLM is already live on **4000**, tick **skipped**. After **`[VPE STANDBY]`**, confirm **Uvicorn** on **4000** → **API is Live**. [API-SetUp-Master.md](../API-SetUp-Master.md).
...
- [ ] Intended branch matches [Checkpoint.md](../guides/Checkpoint.md) (or `git status` is intentional).
...
- [ ] **"start API"** / **Start Project** → **`.\google-api\vpe-start-api.ps1`** + global **`ngrok http 4000`** in **split integrated panes** (no external PowerShell windows); **`.\google-api\gcp_key.json`** + **`.\google-api\litellm_config.yaml`** · **port 4000**. **`[VPE STANDBY]`** then **Uvicorn** on **4000** → **"API is Live"**.
- [ ] Optional: **`CI=true`** + **`npm run test:e2e`** before a big merge (see Custom-Commands).

### Docs honest?

- [ ] If you changed ports, persistence paths, MCP names, or release steps: update **Checkpoint**, **Custom-Commands**, or **`VPE_ENGINE_CAPABILITIES.md`** in the same effort (low drift). For a full doc/rules sweep after a release, say **[Update Docs](../guides/Custom-Commands.md#update-docs)**.
...
- **Smoke steps:** [Custom-Commands.md — mcp sanity check §10](../guides/Custom-Commands.md#extended-checks-added-2026-05-06-night).
...
- **Project capabilities doc:** [VPE_ENGINE_CAPABILITIES.md](VPE_ENGINE_CAPABILITIES.md) — VPE-specific guardrails, hooks/prompts index, and quality gate. Agents should follow it when doing VPE work.
...
**Practical rule:** If the task is **only** VPE/Electron/Next/PM2, **`VPE_ENGINE_CAPABILITIES.md` + .cursorrules + TRUTH** are enough. If the task touches an external product (Vercel deploy, Notion, Jira), enable or reference the matching **Cursor skill** for that product.

---

## 7. Phrases that expand to full procedures

Defined in [Custom-Commands.md](../guides/Custom-Commands.md):
...
- **`Managed project dev`** — catalog **`vpe:toggle-status`**: missing **`node_modules`** + **`package.json`** → **`project-runner`** shell **`install && dev`**; see [Custom-Commands.md — Managed project dev](../guides/Custom-Commands.md#managed-project-dev-v123)
- **`hardened setup`** — clean install / rebuild / optional Playwright / **`repair:ast`** + E2E + lint  
- **`Vader Sync`** — **`npm run vader:sync`**: **`vader:dev -- --success last`**, then **`vader:post-dev-forge`** (snapshot → syntax guard → **`build:win`**). **`npm run vader:clean-sync`**: **`vpe-clean-sync.cjs`** then **`vader:dev`** (close app to continue). **`npm run vader:deploy`**: **`vader:clean-sync`** then **`build:win`**. **Rules:** [VPE-BUILD-PROTOCOL.md](VPE-BUILD-PROTOCOL.md). **Examples:** [Custom-Commands.md — Vader Sync](../guides/Custom-Commands.md#vader-sync).  
...
1. Read .cursor/docs/core/AGENT-BOOT.md and follow authority order (TRUTH → .cursorrules → VPE_ENGINE_CAPABILITIES.md → PRD → package.json).
2. For current branch and recent decisions, read .cursor/docs/guides/Checkpoint.md.
3. For build sequencing (vader:sync, vader:clean-sync, hand-off builds, ASAR + natives), read .cursor/docs/core/VPE-BUILD-PROTOCOL.md.
...
Use [Checkpoint.md](../guides/Checkpoint.md) for durable **project** state. For **personal** daily notes, keep a separate journal outside git or in a private doc—avoid secrets and huge logs in committed files.
