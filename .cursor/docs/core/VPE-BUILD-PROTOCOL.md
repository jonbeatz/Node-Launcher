...
**Related:** [Custom-Commands.md — Vader Sync](../guides/Custom-Commands.md#vader-sync) (phrases / agent steps) · [Custom-Commands.md — rebuild exe](../guides/Custom-Commands.md#rebuild-exe) (full audited release gates) · [Custom-Commands.md — Managed project dev](../guides/Custom-Commands.md#managed-project-dev-v123) (**v1.2.3** catalog **`install && dev`** bootstrap) · [Checkpoint.md](../guides/Checkpoint.md) — **Build v1.9.9** (doc hierarchy + **`google-api`** consolidation), **Build v1.9.8** (**`vpe-bridge`** / **`has_documentation`**), **Build v1.9.7** (orchestration / **`vader:deploy`**), **Build v1.6.0** (dev branch **`VPE-v1.6.x-Dev`** + vault / journal baseline), prior build lines through **v1.2.6**.

**Shipped npm version:** follow root **`package.json`** / preload **`vpeInfo.version`** (currently **1.9.9**).
...
## 4. In-app tooling (v1.1.0+ reference; features accrete through current **`package.json`** — see [Checkpoint — Build v1.9.9](../guides/Checkpoint.md))
...
*My Studio Channel (MSC). Footer / preload **`vpeInfo.version`** matches root **`package.json`** (e.g. **Station Prime v1.9.9**).*
