...
**Dev branch naming:** **`VPE-v1.{minor}.x-Dev`** — increment **`{minor}`** by **1** for each new development line (active git line may still read **`VPE-v1.6.x-Dev`** while **`package.json` `patch`** advances — e.g. **`1.9.6`** — until you cut **`VPE-v1.7.x-Dev`** or rename). Bump **`package.json`** when you open a new minor line (e.g. **`1.6.0`** on **`VPE-v1.6.x-Dev`**). Full rule: [Checkpoint.md — Build v1.9.6](Checkpoint.md) · [Checkpoint.md — Build v1.6.0](Checkpoint.md).
