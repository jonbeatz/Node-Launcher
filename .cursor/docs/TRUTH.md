...
**Shipped release vs. this title:** The **v2.0** label names the *architecture constitution*, not the npm/Electron semver. **Authoritative shipped patch** is root **`package.json` → `version`** (currently **1.9.8** — re-read the file if disputed). Product/UI deltas: **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)** · milestone index: [Checkpoint.md — Build v1.9.8](Checkpoint.md).
