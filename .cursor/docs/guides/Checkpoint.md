...
- **Docs:** **`.cursor/docs/core/`** — constitution & build law (**`TRUTH.md`**, **`VPE-BUILD-PROTOCOL.md`**, **`AGENT-BOOT.md`**, **`VPE_ENGINE_CAPABILITIES.md`**, **`Vader-Project-Engine.md`**). **`.cursor/docs/guides/`** — runbooks (**`START-HERE.md`**, **`Custom-Commands.md`**, **`Stability.md`**, **`PRD.md`**, **`Checkpoint.md`**). Root **`SKILL.md`** removed (merged into **`VPE_ENGINE_CAPABILITIES.md`**).
- **Google API:** **`ngrok.exe`**, **`vpe-start-api.ps1`**, **`litellm_config.yaml`** live under **`google-api/`**; User PATH append targets **`google-api`** (**`scripts/vpe-add-node-launcher-user-path.ps1`**). Run LiteLLM via **`.\google-api\vpe-start-api.ps1`** from repo root.
- **Notes:** **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)** — **Infrastructure Consolidation (v1.9.9)**. **`[VPE STANDBY]`** unchanged.

## Build v1.9.8 — Type-safe `has_documentation` (Clean Forge / IPC alignment)

- **Version:** **`1.9.8`** — **`package.json`**, preload, footer, **`layout.tsx`** → **Station Prime (v1.9.8)**.
- **Types:** **[`src/renderer/types/vpe-ipc.ts`](../../src/renderer/types/vpe-ipc.ts)** — **`VpeHasDocumentation`** = **`number | boolean`**; **`VpeProjectRow`** and dashboard **`Project`** use that for **`has_documentation`** (no **`string`** on the typed boundary). **`vpe-bridge.ts`** re-exports from there.
- **Bridge:** **`msc_rowHasDocumentationEnabled`** handles SQLite **0/1**, booleans, and legacy **`'0'`** via **`Number`**, argument typed **`unknown`**. List + card paperclip use it with **`vault_has_files`**.
- **Store:** SQLite migration **v13** keeps **`has_documentation INTEGER NOT NULL DEFAULT 1`**; JSON persistence normalizes legacy boolean/string values to **0/1** on load.
- **Notes:** **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)** — **Type-Safe Forge (v1.9.8)**. **`[VPE STANDBY]`** unchanged.

## Build v1.9.7 — Orchestration (`vader:deploy`, `vader:clean-sync` reshape)

- **Version:** **`1.9.7`** — **`package.json`**, preload **`vpeInfo.version`**, footer, **`layout.tsx`** → **Station Prime (v1.9.7)**.
- **Scripts:** **`vader:clean-sync`** = **`node scripts/vpe-clean-sync.cjs`** (optional PM2 kill via **`vpe-pm2-kill-optional.cjs`**, delete **`dist/`**, **15s** settle) **then** **`npm run vader:dev`** — interactive verification; **close Electron** to release the shell. **`vader:deploy`** = **`npm run vader:clean-sync && npm run build:win`** — primary path to **`dist/`** **`.exe`** after manual QA (**no** auto **`vpe:take-state-snapshot`** / **`vpe:check-readiness`** / **`vpe:cleanup-dist`**). For the full gated forge tail, use **`vader:sync`** or **`vader:post-dev-forge`**.
- **Notes:** **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)** — **Orchestration Update (v1.9.7)**. **`[VPE STANDBY]`** still emitted from renderer **`VpeUiLayoutProvider`** on shell hydration (unchanged).

## Build v1.9.6 — Shipped semver / dashboard polish (patch train)

- **Version:** **`1.9.6`** — **`package.json`**, preload **`vpeInfo.version`**, footer, **`layout.tsx`** → **Station Prime (v1.9.6)**.
- **Product notes:** **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)** — **v1.9.x** (favorites rail, icon clusters, thumbnail **paperclip** overlay **`#00000066`**, solid **Play/Stop** on cards + list, red **STOP**, **`has_documentation`** / vault reference semantics, list utility hovers).
- **Branch:** Dev branch name may still read **`VPE-v1.6.x-Dev`** until renamed; **patch** versions advance in **`package.json`** on this line regardless.

## Build v1.6.0 — Dev branch `VPE-v1.6.x-Dev` + project vault / journal

- **Version:** **`1.6.0`** — **`package.json`**, preload **`vpeInfo.version`**, footer, **`layout.tsx`**, tray, Prompt Vault master **`versionLabel`** rows in **`vpe-ipc.js`** → **MSC Media Engine v1.6.0**.
- **Branch:** **`VPE-v1.6.x-Dev`** (renamed from **`VPE-v1.5.x-Dev`**; prior **`VPE-v1.4.x-Dev`**). Next new line example: **`VPE-v1.7.x-Dev`**.
- **Vault on disk:** [`src/main/vpe-vault-paths.js`](../../src/main/vpe-vault-paths.js) — default root **`d:\Cursor_Projectz\Node-Launcher\media\vault`** on Windows (override **`VPE_VAULT_ROOT`**); per-project folder from registry **`name`**; optional migrate from legacy **`userData/media/vault`**; **`vpe:save-settings`** renames vault folder when **`name`** changes.
- **IPC:** **`vpe:vault-delete-file`**; preload **`vaultDeleteFile`**.
- **Journal:** SQLite **`notes`** stores JSON **`{ v: 1, entries: [...] }`**; Project Settings **Journal** UI + per-entry delete ([`vpe-project-journal.ts`](../../src/renderer/lib/vpe-project-journal.ts)).

## Build v1.5.0 — Dev branch line `VPE-v1.5.x-Dev` (historical; branch renamed to **`VPE-v1.6.x-Dev`** for v1.6.0) + hygiene / docs alignment

- **Version:** **`1.5.0`** — **`package.json`**, preload **`vpeInfo.version`**, footer, **`layout.tsx`**, tray, Prompt Vault **master** seeds in **`vpe-ipc.js`** → **MSC Media Engine v1.5.0**.
- **Branch:** **`VPE-v1.5.x-Dev`** (prior **`VPE-v1.4.x-Dev`**). Next new line example: **`VPE-v1.6.x-Dev`** (increment **`minor`**, bump **`package.json`** minor, then patch on that line).
- **Hygiene:** Root **`knip.json`** (entry: **`main.js`**, **`preload.js`**, **`app/layout.tsx`**, **`app/page.tsx`**); removed orphan **`src/main/main - Copy.js`**, **`thumbnailer.js`**, **`host-cpu-ticks.js`**; explicit **`prism-react-renderer`** in **`package.json`** for **`Sandbox.tsx`**; **`vpe-ipc.js`** console prefixes **`[VPE ERROR]`** / **`[VPE SUCCESS]`** + IPC response note on **`msc_registerVpeIpc`**; **`@internal`** JSDoc on selected **`msc_*`** re-exports (**`path-guard`**, **`project-detection`**, **`package-json-script-patch`**).

## Build v1.4.0 — Dev branch line & shipped version alignment (`VPE-v1.4.x-Dev`)

- **Version:** **`1.4.0`** — first semver on the **v1.4** dev line; **`package.json`**, preload **`vpeInfo.version`**, footer, **`layout.tsx`**, tray, Prompt Vault **master** seeds in **`vpe-ipc.js`** all match **v1.4.0** / **MSC Media Engine v1.4.0**.
- **Branch naming (living convention):** development branches use **`VPE-v{major}.{minor}.x-Dev`** with **`major`** = **1** for this repo. When opening a **new** dev line, increment **`minor`** by **1** (example: after **`VPE-v1.4.x-Dev`** → **`VPE-v1.5.x-Dev`**). The **`.x`** is a label placeholder only; semver patches ship as **`1.{minor}.{patch}`** in **`package.json`**. **Rule:** root **`package.json` `version`** must match the branch line’s minor (e.g. **`1.4.0`** on **`VPE-v1.4.x-Dev`**), then bump patch for fixes on that line.
- **Carries forward from v1.3.7:** **`build.asarUnpack`** (**`better-sqlite3`**, **`node-pty`**, **`pm2`**) + **`pm2-client.js`** + **`vpe:stop-all`** hardening — unchanged by the branch / label bump alone.

## Build v1.3.7 — ASAR unpack for native modules (better-sqlite3, node-pty, pm2)

- **Version:** **`1.3.7`** — **`package.json`** **`build.asarUnpack`**: **`better-sqlite3`**, **`node-pty`**, **`pm2`**; shipped labels match **v1.3.7** (preload, footer, **`layout.tsx`**, tray, Prompt Vault seeds in **`vpe-ipc.js`**).
- **Rationale:** Native **`.node`** addons cannot load from inside **`app.asar`**; electron-builder copies matched paths to **`resources/app.asar.unpacked`**.
- **PM2 programmatic API:** [`pm2-client.js`](../../src/main/pm2-client.js) **`msc_getPm2()`** — when **`app.isPackaged`**, **`require`** from **`app.asar.unpacked/node_modules/pm2`** (plain **`require('pm2')`** from inside **`app.asar`** can throw *Module not found*). [`pm2-manager.js`](../../src/main/pm2-manager.js) **`stopAll()`** awaits **`msc_ensureConnected()`** before **`pm2.stop('all')`** and resolves **`false`** on failure so **`vpe:stop-all`** still runs runner kill + SQLite cleanup.
- **`vpe:stop-all` hardening:** **`msc_pm2CleanupRegistered`** / **`msc_evictPm2Slot`** require **`msc_ensureConnected()`** before **`pm2.delete`** (avoids hung IPC when RPC never connects); **12s** evict timeout, **20s** **`pm2.stop('all')`** timeout; [`vpe-ipc.js`](../../src/main/vpe-ipc.js) **`try/catch`**, always **`{ ok, error? }`**, **`msc_emitProjectsUpdated`** on failure; renderer error toast if **`ok === false`**. See [Stability](Stability.md) subsection *Stop All / `vpe:stop-all` IPC hang*.

## Build v1.3.6 — Settings wiring, factory defaults, tray close, auto-start restore

- **Version:** **`1.3.6`** — shipped labels (`package.json`, preload **`vpeInfo.version`**, footer, **`layout.tsx`**, tray brand); Prompt Vault **master** rows **`MSC Media Engine v1.3.6`** in **`vpe-ipc.js`** where applicable.
- **Defaults:** **`settings-defaults.js`** + SQLite **`settings`**: **`launch_at_login`**, **`minimize_to_tray`**, **`auto_start_projects`** default **off**; **`default_view`** default **`card`** (dashboard **grid**).
- **Main:** **`vpe-ipc.js`** — **`vpe:get-app-settings`**, **`vpe:update-app-settings`**, **`vpe:update-setting-launch-startup`**; **`app.setLoginItemSettings`**, boot **`msc_runAutoStartProjectsIfEnabled`** (IDs running *before* reconcile → **`startDev`** if reconciled to stopped). **`main.js`** — **`close`** hides when minimize-to-tray; **`before-quit`** sets quit flag; **`will-finish-launching`** applies login item from store.
- **Renderer:** **`app-settings-modal.tsx`** — IPC hydrate + per-toggle save + **Settings saved** toast; **`page.tsx`** boot **`default_view`** vs **`VPE_DASHBOARD_VIEW_LS_KEY`**.
...
- **Branching (historical, v1.3.1):** introduced **`VPE-v1.3.x-Dev`** (replaces **`Node-Launcher-v11`**). **Active git line (may lag semver):** **`VPE-v1.6.x-Dev`** — shipped **`package.json`** may read **1.9.x** on that line; see **Build v1.9.9** above, then **Build v1.6.0**.
...
- **Docs / standards:** [`.cursor/docs/core/VPE-BUILD-PROTOCOL.md`](../core/VPE-BUILD-PROTOCOL.md) — **Skills** (accordion UI density), **Standards** (nav **`#2a2a2a`** lock).
...
- **`vader:clean-sync`:** **v1.9.7+** — **`vpe-clean-sync.cjs`** then **`vader:dev`** only; **`vader:deploy`** appends **`build:win`** after dev exits. **Historical (v1.1.8–v1.9.6):** **`rimraf`** + detached dev + **`post-dev-forge`** via prior **`vpe-clean-sync.cjs`** behavior (superseded). Use **`vader:sync`** for **`--success last`** + full forge tail.
...
- **Forge gate:** **`vader:sync`** and **`vader:dev-to-forge`** end with **`vader:post-dev-forge`**: **`node scripts/vpe-forge-pause.cjs`** (3s) then **`vpe:take-state-snapshot`** (`user-data/auto-snapshots/…-AUTO-PRE-BUILD`) → **`vpe:check-readiness`** (forbidden TS-in-`.js` under **`src/main` + `src/renderer`**) → **`build:win`** → **`vpe:cleanup-dist`**. **`vader:sync`** uses **`--success last`**. **`vader:clean-sync`** (**v1.9.7+**) does **not** auto-run **`post-dev-forge`** — see **Build v1.9.7**. **`npm run vader:dev-to-forge`** runs **`vader:dev`** then the same tail (no **`rimraf`**). **`vader:dev`** sets **`VPE_LAUNCHER_FORGE=1`** (**v1.1.6+** no main-process thermal polling; **v1.1.7+** no **`cpuTemp`** in IPC/UI).
...
- **Docs:** Canonical detail — [VPE-BUILD-PROTOCOL.md](../core/VPE-BUILD-PROTOCOL.md) (v1.1.8). Phrases — [Custom-Commands.md](Custom-Commands.md).
...
**Last doc update:** 2026-05-09 — **v2.0.0 Phase A (doc zero drift):** removed ghost duplicates under **`.cursor/docs/`** root; canon only **`core/`** + **`guides/`** (+ adjunct root refs). **Build v1.9.9** (**`package.json` `1.9.9`**); **`.cursor/docs/core|guides`**, **`google-api/`** launcher paths, **`VPE_ENGINE_CAPABILITIES.md`** per **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)**. **Build v1.9.8:** **`types/vpe-ipc.ts`** + **`vpe-bridge`** **`has_documentation`** Clean Forge. **Build v1.9.7:** **`vader:clean-sync`** / **`vader:deploy`**. **Build v1.9.6:** dashboard / indicator polish. **Update Docs** sweep (README, PRD, START-HERE, AGENT-BOOT, VPE-BUILD, Stability, Custom-Commands, **`.cursorrules`**, **`SKILL.md`**, **Checkpoint**, **TRUTH** semver note). Git branch name may still be **`VPE-v1.6.x-Dev`**. **2026-05-08:** **v1.6.0** dev branch **`VPE-v1.6.x-Dev`** (renamed from **`VPE-v1.5.x-Dev`**) + shipped **`1.6.0`**; project **vault** root [`vpe-vault-paths.js`](../../src/main/vpe-vault-paths.js), **journal** notes JSON, **`vpe:vault-delete-file`**. Prior **v1.5.0** sweep aligned with [START-HERE](START-HERE.md) branch convention; hygiene (**`knip.json`**, removed orphan **`main - Copy` / `thumbnailer` / `host-cpu-ticks`**, **`prism-react-renderer`**, **`vpe-ipc.js`** **`[VPE ERROR]`** / **`[VPE SUCCESS]`** + IPC response note, **`@internal`** JSDoc on selected **`msc_*`** re-exports). **v1.4.0** prior line **`VPE-v1.4.x-Dev`**. **v1.3.7** **`build.asarUnpack`** + **`pm2-client.js`** (**`msc_getPm2`**) for packaged **Stop All** / PM2 RPC; **`vpe:stop-all`** **`msc_ensureConnected`** gates + **12s** evict / **20s** **`stop('all')`** timeouts; IPC always **`{ ok, error? }`** + emit on failure; **`better-sqlite3`**, **`node-pty`**, **`pm2`** outside **`app.asar`**. **v1.3.6** app settings IPC (**`vpe:get-app-settings`**, launch-at-login, minimize-to-tray **close**, auto-start after reconcile, **`default_view`**); **`settings-defaults.js`**; SQLite migration **user_version 9**; JSDoc-only types in **`persistent-store.js`** (syntax guard). **v1.3.5** Top bar **+ Add New Project** + single catalog badge; **flat Dashboard**; **Vault** + **VPE Sandbox**; Maintenance **Prompt Vault** first; **v1.3.4–v1.3.3** Strategist / Vault **`type`**; **CI** relaxed lint. **v1.3.2** ghost watcher + dashboard **localStorage**. **Active branch:** **`git status`**. Pipeline: [Custom-Commands — **rebuild exe**](Custom-Commands.md#rebuild-exe). Backlog: [Stability-Fix-Backlog](Stability-Fix-Backlog.md). **Packaging:** `build.asar = true` + **`asarUnpack`** globs above; **`npm install`** required so **`node-pty`** resolves before **`build:win`**.

## Current project status (snapshot)

| Area | Status |
|------|--------|
| **Branch** | Confirm with **`git status`** — current development line: **`VPE-v1.6.x-Dev`** (prior lines **`VPE-v1.5.x-Dev`** renamed away, **`VPE-v1.4.x-Dev`**, **`VPE-v1.3.x-Dev`**; legacy **`Node-Launcher-v*`** may exist on remotes until pruned). Packaging polish: `src/renderer/out/` gitignored; `prebuild:main` runs static export before **`build:main`**. |
...
| **Native modules** | **v1.3.7+:** **`package.json`** **`build.asarUnpack`** unpacks **`better-sqlite3`**, **`node-pty`**, **`pm2`** to **`app.asar.unpacked`**. **`npm run rebuild:natives`** = `electron-rebuild -f -o better-sqlite3` only (avoids Windows **node-pty** + Spectre MSVC trap); ensure **`node-pty`** is installed (**`npm install`**) before electron-builder. |
...
| **Vault / Sandbox UX (v1.2.9–v1.9.9)** | **v1.9.9:** doc **`core|guides`** hierarchy + **`google-api`** launcher consolidation + **`VPE_ENGINE_CAPABILITIES.md`**. **v1.9.8:** **`types/vpe-ipc.ts`** (**`VpeHasDocumentation`**) + **`vpe-bridge`** coercion + paperclip gate parity (list + card). **v1.9.7:** **`vader:deploy`** / **`vader:clean-sync`** — **[`VADER_STATION_LOG.md`](../../VADER_STATION_LOG.md)**. **v1.9.x:** Dashboard indicators, thumbnail **paperclip**, solid **Play/Stop**, vault reference semantics. **v1.6.0:** Catalog **reference vault** on disk (**`media/vault/<name>/`**, rename on project rename; IPC delete file). **v1.3.5:** **Vault** sidebar group holds **VPE Sandbox**; **Engineer** = accordion; **Add** in **TopBar**; **Prompt Vault** tab first in Maintenance. **v1.3.4:** Strategist accordion; Engineering/Vault/Favorites collapsed by default. **v1.3.3+:** Strategist / Engineer **Tabs**. Accordion vault list + **`vpe:update-vault-item`**; **v1.3.0** collapsed create row (**+ Create New Master Directive**). **Checkpoint:** **Build v1.9.8**, **Build v1.9.7**, **Build v1.9.6**, **Build v1.6.0**, **Build v1.5.0**, **Build v1.4.0**, **Build v1.3.7**, **Build v1.3.6**, **Build v1.3.5**, **Build v1.3.4**, **Build v1.3.3**, **Build v1.2.9**, **Build v1.3.0**. |
...
1. Read this file: `.cursor/docs/guides/Checkpoint.md`. For a **full cold-start index** (authority order, MCP, skills, tick list): `.cursor/docs/core/AGENT-BOOT.md`.
2. Read architecture + constraints:
   - `.cursorrules`
   - `.cursor/docs/guides/PRD.md`
   - `.cursor/docs/core/VPE_ENGINE_CAPABILITIES.md`
...
- **Native CPU / RAM (telemetry path only):** Implemented **inline** in [`vpe-ipc.js`](../../src/main/vpe-ipc.js) using **`os.cpus()`** tick deltas and **`os.totalmem()` / `os.freemem()`**—no optional telemetry packages. First CPU sample returns sentinel **`cpu: -1`** (UI shows **—** until the next ~3s poll). Legacy **`host-cpu-ticks.js`** was removed from the tree (telemetry path does not depend on it).
- **PM2 telemetry without `pm2.list`:** Calling **`pm2.list()`** from telemetry lazy-loads optional deps that can fail under **ASAR** (*Cannot find module* / unhandled rejections). Telemetry **`pm2.status`** is **`online`** only when [`pm2-manager.js`](../../src/main/pm2-manager.js) **`pm2.connect`** has succeeded (`msc_isPm2RpcConnected()`); **`pm2.activeCount`** stays **`0`** here (process list avoided on purpose). **v1.3.7+ (incl. v1.6.0):** operational PM2 loads via [`pm2-client.js`](../../src/main/pm2-client.js) **`msc_getPm2()`** — packaged builds **`require`** from **`app.asar.unpacked/node_modules/pm2`** (see **`build.asarUnpack`**).
...
- **System stats hardening**: [`vpe-ipc.js`](../../src/main/vpe-ipc.js) nested try/catch on **`vpe:get-system-stats`** → `msc_fallbackSystemStats`; CPU % clamping uses **`Number.isFinite`** guards inline on the telemetry path.
...
- CI-parity quick checks:
  ...
  - `npm run test:e2e`
  - `npm run lint`
- Production installer (full gates + clean **`dist/`**): say **rebuild exe** or follow [Custom-Commands — rebuild exe](Custom-Commands.md#rebuild-exe). Minimal: **`npm run build`** / **`npm run build:win`** ( **`build:main`** → **`prebuild:main`** runs icon + **`build:renderer`** once). Dev-then-pack: [Vader Sync](Custom-Commands.md#vader-sync). Unified rules: [VPE-BUILD-PROTOCOL.md](../core/VPE-BUILD-PROTOCOL.md).
