...
**Related:** [Custom-Commands.md — Vader Sync](Custom-Commands.md#vader-sync) (phrases / agent steps) · [Custom-Commands.md — rebuild exe](Custom-Commands.md#rebuild-exe) (full audited release gates) · [Custom-Commands.md — Managed project dev](Custom-Commands.md#managed-project-dev-v123) (**v1.2.3** catalog **`install && dev`** bootstrap) · [Checkpoint.md](Checkpoint.md) — **Build v1.9.8** (**`vpe-bridge`** / **`has_documentation`**), **Build v1.9.7** (orchestration / **`vader:deploy`**), **Build v1.9.6**, **Build v1.6.0** (dev branch **`VPE-v1.6.x-Dev`** + vault / journal baseline), **Build v1.5.0**, **Build v1.4.0** (prior dev lines), **Build v1.3.7** (ASAR **`asarUnpack`** natives), **Build v1.3.6** (settings IPC, defaults, tray close, auto-start), **Build v1.3.5** (TopBar add, flat Dashboard, Vault + Sandbox, Engineer accordion), **Build v1.3.4** (top bar badge, Strategist accordion), **Build v1.3.3** (Strategist tabs + Vault **`type`**), **Build v1.3.2** (ghost UX), **Build v1.3.1** (CI / branch), **Build v1.3.0**, **Build v1.2.9**, **Build v1.2.6** (archive / jump search).

**Shipped npm version:** follow root **`package.json`** / preload **`vpeInfo.version`** (currently **1.9.8**).
...
- **v1.3.7:** **Packaging** — **`build.asarUnpack`** includes **`better-sqlite3`**, **`node-pty`**, **`pm2`** under **`app.asar.unpacked`**. **Main:** [`pm2-client.js`](../../src/main/pm2-client.js) **`msc_getPm2()`** when **`app.isPackaged`** (PM2 **`require`** path); [`pm2-manager.js`](../../src/main/pm2-manager.js) **`stopAll`** awaits RPC connect before **`pm2.stop('all')`**.
...
| **`npm run vader:clean-sync`** | **Nuke `dist` + interactive dev** | **`node scripts/vpe-clean-sync.cjs`** (optional PM2 kill, delete **`dist/`**, settle delay) **then** **`npm run vader:dev`** in the **same** terminal. **No** automatic **`vader:post-dev-forge`** — close Electron to end **`vader:dev`**, then run **`vader:deploy`** or **`vader:sync`** / **`vader:post-dev-forge`** if you need snapshot + syntax + cleanup-dist. |
| **`npm run vader:deploy`** | **Primary production hand-off** | **`npm run vader:clean-sync && npm run build:win`**. Runs clean + **`vader:dev`** for manual verification; when dev exits, **`build:win`** produces **`dist/`** artifacts. Does **not** run **`vpe:take-state-snapshot`** / **`vpe:check-readiness`** / **`vpe:cleanup-dist`** — use **`vader:sync`** for the full audited forge tail. |
...
- **Strict forge sequence:** **`vader:sync`** and **`vader:dev-to-forge`** end in **`vader:post-dev-forge`**: **3s pause** (**`node scripts/vpe-forge-pause.cjs`**) → Snapshot → Syntax guard → Build → **`vpe:cleanup-dist`**. **`vader:clean-sync`** (**v1.9.7+**) runs **`vpe-clean-sync.cjs`** then **`vader:dev`** only — interactive verification; use **`vader:sync`** or **`vader:post-dev-forge`** for the gated tail. **`vader:deploy`** runs **`vader:clean-sync`** then **`build:win`** after dev exits (**no** snapshot/syntax/cleanup-dist unless you add them manually).
- **Sequential `&&` chains:** **`vader:sync`**, **`vader:dev-to-forge`**, and **`vader:post-dev-forge`** rely on **`&&`**. **`build:win` / `build:main`** run inside **`vader:post-dev-forge`** (after snapshot + syntax) **or** at the end of **`vader:deploy`** (direct **`build:win`** only). **`vader:sync`** requires **`vader:dev`** to exit **0** for the chain to reach **`post-dev-forge`**. **`vader:clean-sync`** chains **`vpe-clean-sync.cjs`** → **`vader:dev`**; **`vader:deploy`** appends **`build:win`** after that **`vader:dev`** process exits.
...
- **Cleanup before big releases:** For version bumps or large UI/asset changes — use **`npm run vader:deploy`** for clean + dev verification + **`build:win`**, or **`npm run vader:sync`** when you need **`--success last`** + snapshot + syntax guard + **`vpe:cleanup-dist`**. **`vader:clean-sync`** deletes **`dist/`** then opens **`vader:dev`** only (see master table).
- **ASAR & native rebuild:**
...
## 4. In-app tooling (v1.1.0+ reference; features accrete through current **`package.json`** — see [Checkpoint — Build v1.9.8](Checkpoint.md))
...
*My Studio Channel (MSC). Footer / preload **`vpeInfo.version`** matches root **`package.json`** (e.g. **Station Prime v1.9.8**).*
