...
- **`vpe:stop-all` hardening:** **`msc_pm2CleanupRegistered`** / **`msc_evictPm2Slot`** require **`msc_ensureConnected()`** before **`pm2.delete`** (avoids hung IPC when RPC never connects); **12s** evict timeout, **20s** **`pm2.stop('all')`** timeout; [`vpe-ipc.js`](../../src/main/vpe-ipc.js) **`try/catch`**, always **`{ ok, error? }`**, **`msc_emitProjectsUpdated`** on failure; renderer error toast if **`ok === false`**. See [Stability-Fix-Backlog](Stability-Fix-Backlog.md) subsection *Stop All / `vpe:stop-all` IPC hang*.
...
- **Branching (historical, v1.3.1):** introduced **`VPE-v1.3.x-Dev`** (replaces **`Node-Launcher-v11`**). **Active git line (may lag semver):** **`VPE-v1.6.x-Dev`** — shipped **`package.json`** may read **1.9.x** on that line; see **Build v1.9.8** above, then **Build v1.6.0**.
